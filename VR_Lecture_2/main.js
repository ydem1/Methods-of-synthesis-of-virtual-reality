...
function slerp(q1, q2, t) {
    ...
    if (dot > 0.9995) {
        ...
        const len = Math.hypot(result[0], result[1], result[2], result[3]);
...
function draw() { 
    ...
    if (usePhoneRotation) {
        let [qx, qy, qz, qw] = phoneQuaternion;

        let invQ = [-qx, -qy, -qz, qw];

        const factor = 2;
        const boosted = [invQ[0] * factor, invQ[1] * factor, invQ[2] * factor, invQ[3]];
        const len = Math.hypot(boosted[0], boosted[1], boosted[2], boosted[3]);
        const targetQ = boosted.map(v => v / len);

        smoothQuaternion = slerp(smoothQuaternion, targetQ, SMOOTHING_FACTOR);

        const phoneRotation = m4.identity();
        m4.compose([0, 0, 0], smoothQuaternion, [1, 1, 1], phoneRotation);

        modelView = m4.multiply(phoneRotation, modelView);
    }
